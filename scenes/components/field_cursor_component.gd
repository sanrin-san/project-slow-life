...
func add_tilled_soil_cell() -> void:
	if distance < 20.0 && cell_source_id != -1:
		tilled_dirt_tilemap_layer.set_cells_terrain_connect([cell_position], terrain_set, terrain, true)

func remove_tilled_soil_cell() -> void:
	if distance < 20.0:
		tilled_dirt_tilemap_layer.set_cells_terrain_connect([cell_position], 0, -1, true)
